...
import { IFieldMode, IRuleAmbit } from 'src/app/common/models/interfaces';
import { IconModule } from 'src/app/icon/icon.module';

@Component({
  selector: 'rule-ambit-filter',
  templateUrl: './rule-ambit-filter.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => RuleAmbitFilterComponent),
    multi: true,
  }],
  imports: [
    CommonModule,
 ...
 export class RuleAmbitFilterComponent implements ControlValueAccessor, OnInit {
  ...
  control = new FormControl<IRuleAmbit | string | null>(null);

  items: IRuleAmbit[] = [];
  filteredItems: IRuleAmbit[] = [];
  ...
  displayName = (entity: IRuleAmbit | string | null): string =>
    typeof entity === 'object' && entity ? entity.description : '';

  private getItems(): void {
    this.service.getRuleAmbits().subscribe(data => {
      this.items = data;
  ...
  private filter(value: IRuleAmbit | string | null): IRuleAmbit[] {
    const text =
      typeof value === 'string'
        ? value.toLowerCase()
...
/* export class RuleAmbitFilterComponent implements OnInit {

  private legislationService = inject(LegislationService)

  protected items = signal<IRuleAmbit[]>([]);
  protected filteredItems = signal<IRuleAmbit[]>([]);
  selectedItem = model<IRuleAmbit | null>(null);
  @Output() selected: EventEmitter<IRuleAmbit> = new EventEmitter<IRuleAmbit>();
  editable = input<boolean>(true);
  filtering = input<boolean>(true);

  protected control = new FormControl<IRuleAmbit | string>('');
  ...
  private _getFilterValue(value: IRuleAmbit | string | null): string {
  ...
  private _filter(value: IRuleAmbit | string | null): IRuleAmbit[] {
