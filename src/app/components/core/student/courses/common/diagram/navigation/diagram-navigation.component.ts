...
import { Component, ElementRef, inject, Input, OnChanges, OnInit, output, Renderer2, signal, SimpleChanges, ViewChild } from '@angular/core';
...
import { FeatureType, IncomingNavigableEntity, IDiagram } from 'src/app/common/models/interfaces';
...
@Component({
  ...
  templateUrl: './diagram-navigation.component.html',
  styleUrl: './diagram-navigation.component.scss'
})
export class DiagramNavigationComponent implements OnInit, OnChanges {
  ...
  featureType: FeatureType = 'DIAGRAM';
  ...
  selectedDiagram = signal<IDiagram | null>(null);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.articleId)
      this.goNext();
    else this.selectedDiagram.set(null);
  }
  ...
  async update() {
    const currentSnippet = this.selectedDiagram()?.snippet;
    if (currentSnippet) {
  ...
  goNext() {
    this.service.navigate(this.articleId, { diagramId: this.navigationState.nextId! })
      .subscribe(res => {
        this.selectedDiagram.set(res.item as IDiagram);
  ...
  goPrevious() {
    this.service.navigate(this.articleId, { diagramId: this.navigationState.previousId! })
      .subscribe(res => {
        this.selectedDiagram.set(res.item as IDiagram);
        this.navigationState = res;
        this.update();
  ...
  deleteItem() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 'delete') {
        this.service.delete(this.selectedDiagram()?.id || 0).subscribe({
          next: (res) => {
            this.goNext();
  ...
  updateItem() {
    ...
    const id: string = diagram?.id?.toString() || '';
    this.router.navigate([`${this.activatedRouter?.snapshot.data['role'].toLowerCase()}/diagrams/:diagramId/edit`.replace(':diagramId', id)]);
    //if (diagram) {
      //this.store.dispatch(setSelectedDiagram(diagram))
      //this.router.navigate([`${this.activatedRouter?.snapshot.data['role'].toLowerCase()}/diagrams/:diagramId/edit`.replace(':diagramId', id)]);
    //}
  }
